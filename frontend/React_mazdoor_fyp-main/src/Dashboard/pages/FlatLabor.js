...
export default function FlatLabor() {
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8800/jobs/unassignedFlatJobsforlabor', {
          method: 'POST',
          headers: {
  ...
  const handleConfirm = async (jobId) => {
    try {
      const requestData = {
        jobId : jobId,
        laborId : globalUser.userModel._id,
      };

      const response = await fetch('http://localhost:8800/jobs/assignedflatjob', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <TableBody>
            {jobHistory.map((job, index) => (
              <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                <TableCell>
                  <Button variant="outlined" color="primary" sx={{ textTransform: 'none' }} onClick={() => handleConfirm(job.jobId )}>Start job</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
